...
class lfFilterForm(forms.Form):
    lowerVal = forms.DecimalField(
        ...
        required=False,
        max_value=80.0, min_value=-50.0, max_digits=3,
        decimal_places=1)

    upperVal = forms.DecimalField(
        label="Bitte obere Grenze eingeben:",
        widget=forms.NumberInput(attrs={"class": "form-control form-control-lg"}),
        max_value=80.0, min_value=-50.0, max_digits=3, decimal_places=1,
        required=False,
        #initial=23.0
        )
    ...
    vonDate = forms.DateField(
        ...
        label="Bitte das Datum eingeben, ab wann die Luftfeuchtigkeit angezeigt werden sollen. (Default = vor 5 Tagen) ",
        required=False,
        input_formats=['%d.%m.%Y'],
        )
    ...
    bisDate = forms.DateField(
        ...
        widget=forms.DateInput(attrs={"class": "form-control form-control-lg"}, format='%d.%m.%Y'),
        label="Bitte das letzte Datum eingeben, bis zu dem Die Luftfeuchtigkeit angezeigt werden sollen. (Default = heute)",
        input_formats=['%d.%m.%Y'],
        required=False,
        )
